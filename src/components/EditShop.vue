<template>
  <div class="container build myshop yuyue-wrap">
      <div class="wrap" v-if="barberList.length > 0">
          <div class="shop-wrap">
              <div class="shop-layer">
                  <div class="weui-panel__bd" :style="{paddingTop: noticeClosed?'0':'30px'}">
                      <!--item-->
                      <div  class="weui-media-box weui-media-box_appmsg">
                          <div class="weui-media-box__hd">
                              <img class="weui-media-box__thumb"
                                   src="data:image/png;base64,"
                                   alt="">
                          </div>
                          <div class="weui-media-box__bd">
                              <h4 class="weui-media-box__title">标题二<a href="#" class="oper"><i class="ico-edit"></i> </a>
                              </h4>

                              <div class="weui-media-box__info"><span><em class="statu statu-dis">休息中</em></span>

                                  <a href="javascript:;" class="weui-btn weui-btn_primary">预约</a></div>
                          </div>
                      </div>
                      <div class="add-wrap"><a href="#" class="add">添加理发师</a></div>
                  </div>
              </div>
          </div>
          <p class="notic" v-if="!noticeClosed">
              <i class="ico-notic"></i><span>当前页面为你客户看到的效果。</span>
              <a href="javascript:;" @click="closeTips" class="oper"><i
                  class="iconfont-dasan-9"></i> </a>
          </p>
      </div>

      <div v-else class="wrap">
           <p class="notic" v-if="!noticeClosed">
              <i class="ico-notic"></i><span>您的店铺已生成，请添加理发师</span><a href="javascript:;" @click="closeTips" class="oper"><i class="iconfont-dasan-9"></i> </a>
          </p>
          <div class="content wrap" :style="{paddingTop: noticeClosed?'70px':'140px'}">
              <router-link href="javascript:;" class="no-add" to="EditProduct">
                <i class="ico-addprodcut"></i>
                <span>添加理发师</span>
              </router-link>
          </div>
      </div>
      <!--footer-->
      <footer>
          <div class="weui-tabbar">

              <a href="javascript:;" class="weui-tabbar__item weui-bar__item_on">
                  <i class="weui-tabbar__icon iconfont iconfont-dasan-28"></i>

                  <p class="weui-tabbar__label">首页</p>
              </a>
              <a href="javascript:;" class="weui-tabbar__item">
                  <i class="weui-tabbar__icon iconfont iconfont-dasan-3"></i>

                  <p class="weui-tabbar__label">预览小铺</p>
              </a>
          </div>
      </footer>
  </div>
</template>

<script>
export default {
  name: 'hello',
  data () {
    return {
      noticeClosed: false,
      barberList: []
    }
  },
  activated () {
    document.title = '店铺装修'
    this.getBarbers()
  },
  methods: {
    closeTips () {
      this.noticeClosed = true
    },
    getBarbers () {

    }
  }
}
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style lang="less" scoped>
  @import '../assets/less/toc-index.less';
  @import '../assets/less/tob-build.less';
  @import '../assets/less/yuyue.less';
</style>
